...
class TestRedirectMiddleware(unittest.TestCase):
    def setUp(self):
        raw_redirects = (
                {'old': '/foo/', 'new': '/bar/', 'status_code': None, 'filename': '', 'line_number':1},
                {'old': '/foo/302/', 'new': '/bar/', 'status_code': 302, 'filename': '', 'line_number':1},
                {'old': '/foo/303/', 'new': '/bar/', 'status_code': 303, 'filename': '', 'line_number':1},
                {'old': 'http://www.fusionbox.com/asdf/', 'new': '/foo/bar/', 'status_code': None, 'filename': '', 'line_number':1},
                )
    ...
    def test_410_gone(self):
        path = '/bar/'
        redirects = (
                {'old': '/bar/', 'new': None, 'status_code': None, 'filename': '', 'line_number':1},
            )
    ...
    def test_circular_redirect(self):
        redirects = (
                {'old': '/bar/', 'new': '/bar/', 'status_code': None, 'filename': '', 'line_number':1},
            )
        with self.assertRaises(ImproperlyConfigured):
            preprocess_redirects(redirects)
        redirects = (
                {'old': '/bar/', 'new': '/baz/', 'status_code': None, 'filename': '', 'line_number':1},
                {'old': '/foo/', 'new': '/bar/', 'status_code': None, 'filename': '', 'line_number':1},
            )
        with self.assertRaises(ImproperlyConfigured):
            preprocess_redirects(redirects)
    ...
    def test_domain_circular_redirect(self):
        redirects = (
                {'old': 'http://www.fusionbox.com/bar/', 'new': '/bar/', 'status_code': None, 'filename': '', 'line_number':1},
            )
        with self.assertRaises(ImproperlyConfigured):
            preprocess_redirects(redirects)
        redirects = (
                {'old': 'http://www.fusionbox.com/bar/', 'new': '/foo/', 'status_code': None, 'filename': '', 'line_number':1},
                {'old': 'http://www.google.com/foo/', 'new': '/asdf/', 'status_code': None, 'filename': '', 'line_number':1},
            )
        preprocess_redirects(redirects)
        redirects = (
                {'old': 'http://www.fusionbox.com/foo/', 'new': '/asdf/', 'status_code': None, 'filename': '', 'line_number':1},
                {'old': 'http://www.fusionbox.com/bar/', 'new': '/foo/', 'status_code': None, 'filename': '', 'line_number':1},
            )
        with self.assertRaises(ImproperlyConfigured):
            preprocess_redirects(redirects)
    ...
    def test_duplicate_redirect(self):
        redirects = (
                {'old': '/bar/', 'new': '/baz/', 'status_code': None, 'filename': '', 'line_number':1},
                {'old': '/bar/', 'new': '/asdf/', 'status_code': None, 'filename': '', 'line_number':1},
            )
    ...
    def test_possible_circular_redirect(self):
        redirects = (
                {'old': '/bar/', 'new': 'http://fusionbox.com/foo/', 'status_code': None, 'filename': '', 'line_number':1},
                {'old': '/foo/', 'new': '/asdf/', 'status_code': None, 'filename': '', 'line_number':1},
            )
        with warnings.catch_warnings(record=True) as w:
            preprocess_redirects(redirects)
            assert len(w)
        redirects = (
                {'old': '/foo/', 'new': 'http://fusionbox.com/foo/', 'status_code': None, 'filename': '', 'line_number':1},
            )
        with warnings.catch_warnings(record=True) as w:
            preprocess_redirects(redirects)
            assert len(w)

    def test_redirect_to_filename(self):
        redirects = (
                {'old': '/bar/', 'new': '/bar/asdf.pdf', 'status_code': None, 'filename': '', 'line_number':1},
            )
        preprocess_redirects(redirects)
        with warnings.catch_warnings(record=True) as w:
            preprocess_redirects(redirects)
    ...
    def test_redirect_to_non_filename_no_slash_url(self):
        redirects = (
                {'old': '/bar/', 'new': '/bar/asdf', 'status_code': None, 'filename': '', 'line_number':1},
            )
        with warnings.catch_warnings(record=True) as w:
            preprocess_redirects(redirects)
            assert len(w)
    ...
    def test_cross_domain_redirect(self):
        redirects = (
                {'old': 'http://www.fusionbox.com/bar/', 'new': 'http://www.google.com/bar/', 'status_code': None, 'filename': '', 'line_number':1},
            )
        preprocess_redirects(redirects)
